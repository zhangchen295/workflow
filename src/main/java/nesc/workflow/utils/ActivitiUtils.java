...
import nesc.workflow.config.DeleteTaskCmd;
import nesc.workflow.config.SetFLowNodeAndGoCmd;
...
public class ActivitiUtils {
        ...
        try {
            if(flowIds == null
                ...
                imageStream = new DefaultProcessDiagramGenerator()
                        .generateDiagram(bpmnModel,"宋体","微软雅黑","黑体");
            }else {
                imageStream = new DefaultProcessDiagramGenerator()
    ...
    public void showImg(String instanceId, HttpServletResponse response) {
        ...
        if (processInstance == null) {
            LOGGER.error("process instance {} not exist", instanceId);
            return;
        }
        ...
        if (historicActivityInstanceList == null || historicActivityInstanceList.size() == 0) {
            LOGGER.info("process instance history node info not exist", instanceId);
        ...
        //处理撤回这种情况 根据id排序
        List<Task> tasks = taskService.createTaskQuery().processInstanceId(instanceId).orderBy(TaskQueryProperty.TASK_ID).asc().list();
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public void backTask(String currentTaskId, String targetTaskId) throws Exception {
        //准备数据
        ...
        if (sequenceFlows.size() == 1) {
            ...
            if (historicActivityInstance.getActivityType().equals(PARALLEL_GATEWAY)) {
                ...
                if (!parentFlowNodes.isEmpty()) {
                    ...
                    //没有父级任务，图有问题
                    throw new Exception("bpmn doc error");
                }
                ...
                //todo 还没想好这种场景
                throw new Exception(BPMN_NOT_SUPPORT);
            }
        } else {
            //当前节点的上级节点有多条 这里需要指定要回退的taskId
    ...
    private void validatorTargetTask(Map<String, HistoricTaskInstance> historicTaskInstanceMap, String targetTaskId) throws Exception {
        if (StringUtils.isEmpty(targetTaskId) || StringUtils.isBlank(targetTaskId)) {
            throw new Exception("target task id cannot be null");
        }
        if (historicTaskInstanceMap == null || historicTaskInstanceMap.isEmpty()) {
            throw new Exception("historic task instance cannot be null");
        }
    }

    @Transactional(rollbackFor = Exception.class)
    public void deleteTaskMultiple(Map<String, FlowNode> flowNodeMap, Map<String, HistoricTaskInstance> historicTaskInstanceMap, String targetTaskId, String targetTaskDefinitionKey, Task currentTask, List<Task> taskList, String targetParentTaskDefinitionKey) throws Exception {
